...
- The list is server-provided (`GET /models` → `{models:[{id,label,model,providerId?,providerLabel,cost,note,metered}], groups:[...]}`), never hard-coded in a screen. `FALLBACK_MODELS` covers the gap before the endpoint answers — a flat, single-provider (Anthropic) list.
...
- A row's `note` is exactly what the server wrote and is rendered verbatim — never re-derived from `cost`/`metered` here. That is what keeps an unmetered (`metered: false`) row from ever growing a dollar figure it doesn't have; the wording lives entirely on the server, one place, in `describeModel()`.
