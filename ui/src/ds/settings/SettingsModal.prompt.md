...
- Projects also holds **Isolation** (`Shared checkout` | `A worktree per mission`) and **Missions at once** (1–5). They read as one decision: a shared checkout is one folder, so it is always one mission; a worktree project gives each mission its own checkout under Foreman's home and can run several. Worktrees need a git repository — with `projectIsRepo={false}` the choice is shown greyed with the reason, not hidden, and the number row is greyed at 1 whenever isolation is `shared`. The server refuses both cases as well; this is the friendly half.
