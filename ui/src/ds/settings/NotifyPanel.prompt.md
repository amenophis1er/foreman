...
- **Linking is one scan.** While a code is open the panel shows a QR of `https://t.me/<bot>?start=<code>` (served by `GET /notify/telegram/qr.svg`, only while linking) beside the typed command, plus an `Open in Telegram on this computer →` anchor. Telegram delivers the `start` payload as exactly the `/start <code>` message the server waits for, so scanning and typing are the same act. The QR sits on a white tile — QR readers need the contrast, whatever the theme.
