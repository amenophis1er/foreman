...
- Warning border and `--brand-wash-strong` fill, pulsing with the `.pulse` class — the same warning signal `StatusBadge` uses for `needs-you`, at full width, because this is the one thing on the page that wants you.
