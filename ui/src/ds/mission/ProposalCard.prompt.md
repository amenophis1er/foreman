...
- **Below the criteria sits the Composer's row, in the Composer's order: Budget cap · Director · Workers · Browser, then Crew.** The two ways of starting a mission must never disagree about what can be chosen. Models default to inherit (the project's Settings), like the Composer.
...
- **The crew row is the Composer's, shared verbatim** (`CrewToggles`): the same toggles, under the same tray, hidden entirely when no presets are configured. A proposal may carry `crew` — preset ids the planner suggests — and that only sets where the toggles start; the human's toggles decide what is sent.
- `onStart` receives the full `ProposalStart` — brief, budget, both models with their provider ids, `browserTools` and `crew` — not just brief and budget.
